  ...
  const r = await fetchWithTimeout(
    "https://openrouter.ai/api/v1/chat/completions",
    {
      ...
      headers: {
        ...
        "HTTP-Referer": process.env.VERCEL_URL || "https://roots-app-gamma.vercel.app",
        "X-Title": "Roots App",
      },
      body: JSON.stringify({
        model,
        messages: [{ role: "user", content: prompt + "\n\nReturn ONLY valid JSON." }],
        max_tokens: Math.min(maxTokens || 400, 1200),
        temperature: 0.2,
      }),
    },
    9000 // bajo límite Vercel Hobby
  );
...
export default async function handler(req, res) {
  ...
  // `force`: "groq" | "gemini" | "openrouter" — el frontend lo usa para reintentar
  // un JSON malformado con OTRO proveedor (piensa distinto y suele formatear mejor).
  if (force && force !== "groq" && force !== "gemini" && force !== "openrouter") {
  ...
  const tryOpenRouter = async () => ok(await callOpenRouter(prompt, max_tokens), "openrouter");
  ...
  if (force === "openrouter") {
    if (!process.env.OPENROUTER_API_KEY) return res.status(500).json({ error: { message: "OPENROUTER_API_KEY no configurada" } });
    try { return await tryOpenRouter(); }
    catch (e) { return res.status(502).json({ error: { message: `OpenRouter falló: ${e.message}` } }); }
  }
